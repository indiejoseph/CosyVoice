...
def get_args():
    ...
    parser.add_argument("--beta", default=0.01, type=float, help="beta of dpo training")
...
def main():
    ...
    override_dict["qwen_pretrain_path"] = os.path.join(
        os.path.dirname(args.config), "CosyVoice-BlankEN"
    )
    ...
    if args.dpo:
        ref_model, _, _, _, _ = init_optimizer_and_scheduler(
            args, configs, ref_model, gan
        )
