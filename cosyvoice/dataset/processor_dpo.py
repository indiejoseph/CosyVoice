...
def csv_opener(data, mode="train", tts_data={}):
    for sample in data:
        assert "src" in sample
        line = sample["src"].strip()

        if line == "prompt,chosen,rejected":  # skip header
            continue

        reader = csv.reader(StringIO(sample["src"]), quotechar='"')
        fields = next(reader)
        text = fields[0]
        speech_token_list = [int(i) for i in fields[1].split(" ")]
        reject_speech_token_list = [int(i) for i in fields[2].split(" ")]
        sample["utt"] = text
        sample["text"] = text
        sample["wav"] = "not available"
        sample["audio_data"] = "not available"
        sample["spk"] = "not available"
        sample["speech"] = torch.zeros(1, 1)
        sample["utt_embedding"] = torch.zeros(192)
        sample["spk_embedding"] = torch.zeros(192)
        sample["speech_feat"] = torch.zeros(80, 100)
        sample["speech_token"] = speech_token_list
        sample["reject_speech_token"] = reject_speech_token_list
